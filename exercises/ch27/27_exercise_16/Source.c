...
#define SET_VFPOINTER(shape_p, fct_p, dfct_p) shape_p->shape.vft->fct_p = &dfct_p
...
{
    struct Circle *c = REINTERPRET_CAST(struct Circle *, p);
...
struct Circle *init_circle(int x, int y, int r)
{
    ...
    SET_VFPOINTER(circle, draw, draw_circle);
    SET_VFPOINTER(circle, rotate, rotate_circle);
    circle->r = r;
    return circle;
}
