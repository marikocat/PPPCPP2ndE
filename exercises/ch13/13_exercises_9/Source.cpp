...
try {
	...
	Simple_window win{ Point{100, 100}, 1100, 800, "Regular hexagon" };
	...
	for (int i = 0; i < 4; ++i)
		for (int j = 0; j < 4; ++j)
		{
			hexVector.push_back(new Regular_hexagon{ Point{radius + i * radius * 3,
				int(radius * cos(30 * pi / 180)) + j * int(radius * cos(30 * pi / 180)) * 2}, radius });
	...
	for (int i = 0; i < 3; ++i)
		for (int j = 0; j < 4; ++j)
		{
			hexVector.push_back(new Regular_hexagon{ Point{radius / 2 * 5 + i * radius * 3,
				int(radius * cos(30 * pi / 180)) * 2 + j * int(radius * cos(30 * pi / 180)) * 2}, radius });
			hexVector[hexVector.size() - 1].set_color(Color::black);
			win.attach(hexVector[hexVector.size() - 1]);
		}
