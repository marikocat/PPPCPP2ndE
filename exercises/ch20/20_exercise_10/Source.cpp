...
bool char_is_in_set(const char& ch, const string& s)
{
	for (char c : s)
		if (ch == c) return true;
	return false;
...
int count_words(Document& d, const string& s)
{
	int res = 0;
	Text_iterator p = d.begin();
	bool was_space = false;
	for (; p != d.end(); ++p)
		if (char_is_in_set(*p, s))
		{
			if (!was_space) res++;
			was_space = true;
		}
...
int main()
{
	...
	string s;
	cout << "Enter the set of whitespace characters:\n";
	...
	cout << "The number words is " << count_words(d, s) << '\n';
}