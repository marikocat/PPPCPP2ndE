...
namespace Graph_lib {
	...
	Bar_graph::Bar_graph(Point origin, double wscale, double hscale, const vector<Bar_data>& values)
	{
		...
		{
			cout << "x1 = " << point(0).x + int(i * wscale) << "; y1 = " << point(0).y - int(hscale * values[i].people_num) << "\n";
			cout << "x2 = " << point(0).x + int((i + 1) * wscale) << "; y2 = " << point(0).y << "\n";
			v.push_back(new Bar{
...
try
{
	...
	const int max_height = 78;
	const int height_gap = 1;

	const int max_people_num = 30;

	const double xscale = double(xlength) / ((max_height - min_height) / height_gap + 1);
	const double yscale = double(ylength) / max_people_num;
	...
	x.label.move(xlength - xoffset, 0);
	...
	for (int i = 0; i <= max_people_num / 10; ++i)
	{
		num_labels.push_back(new Text{ {xoffset - 20, ymax - yoffset - int(i * 10 * yscale)}, value_to_string(i * 10) });
